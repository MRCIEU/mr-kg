...
from app.services.database_service import StudyService, AnalyticsService
...
class StudyDetailExtended(BaseModel):
    ...
    study: ModelResult
    pubmed_data: Optional[MRPubmedData] = None
...
async def list_studies(
    ...
    order_by: str = Query(default="pub_date", description="Sort field: pub_date, trait_count, pmid"),
    ...
    try:
        ...
        valid_order_fields = ["pub_date", "trait_count", "pmid", "model", "journal"]
        ...
        if order_by == "trait_count":
            ...
            order_clause = f"mpd.{order_by} {order_direction}" if order_by in ["pub_date", "journal"] else f"mr.{order_by} {order_direction}"
...
async def search_studies(
    q: str = Query(..., description="Search query for PMID, title, or abstract"),
    page: int = Query(default=1, ge=1, description="Page number (1-based)"),
    page_size: int = Query(default=50, ge=1, le=200, description="Items per page"),
    model: Optional[str] = Query(default=None, description="Filter by model"),
    journal: Optional[str] = Query(default=None, description="Filter by journal"),
    ...
    try:
        ...
        params: List[Union[int, str]] = [search_pattern, search_pattern, search_pattern]
        ...
        raise HTTPException(status_code=500, detail=f"Failed to search studies: {str(e)}")


@router.get("/{study_id}", response_model=DataResponse[StudyDetailExtended])
async def get_study_details(
    study_id: int,
    include_similar: bool = Query(default=True, description="Include similar studies"),
    include_traits: bool = Query(default=True, description="Include associated traits"),
    max_similar: int = Query(default=10, ge=1, le=50, description="Maximum similar studies"),
    ...
    try:
        ...
        """
        stats_result = service.study_repo.execute_query(stats_query, (study_id,))
        statistics = {
        ...
        similar_studies = study_detail.similar_studies if include_similar else []
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get study details: {str(e)}")
...
async def get_studies_by_pmid(
    ...
    try:
        ...
        if not studies:
            raise HTTPException(status_code=404, detail=f"No studies found for PMID {pmid}")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get studies for PMID: {str(e)}")


@router.get("/{study_id}/similar", response_model=DataResponse[List[SimilaritySearchResult]])
async def get_similar_studies(
    ...
    try:
        ...
        combination = service.similarity_repo.find_combination(study.pmid, study.model)
        if not combination:
        ...
        results = [
            SimilaritySearchResult(
                ...
                similarity=sim.trait_profile_similarity if similarity_type == "trait_profile" else sim.trait_jaccard_similarity,
            )
        ...
        raise HTTPException(status_code=500, detail=f"Failed to find similar studies: {str(e)}")
...
async def get_studies_overview(
    ...
    try:
        ...
        ORDER BY 1
        """
        trait_count_results = service.study_repo.execute_query(trait_count_query)
        trait_count_distribution = {row[0]: row[1] for row in trait_count_results}
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get studies overview: {str(e)}")
...
async def get_available_journals(
    limit: int = Query(default=100, ge=1, le=500, description="Maximum number of journals to return"),
    service: StudyService = Depends(get_database_service),
) -> DataResponse[List[str]]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get available journals: {str(e)}")