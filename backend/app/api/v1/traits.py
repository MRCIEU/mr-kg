...
async def list_traits(
    ...
    order_by: str = Query(default="appearance_count", description="Sort field: appearance_count, trait_label"),
    order_desc: bool = Query(default=True, description="Sort descending"),
    min_appearances: Optional[int] = Query(default=None, description="Minimum appearance count"),
    ...
    try:
        ...
        total_count = service.trait_repo.execute_query(count_query, tuple(params))[0][0]
        ...
        raise HTTPException(status_code=500, detail=f"Failed to list traits: {str(e)}")
...
async def search_traits(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to search traits: {str(e)}")


@router.get("/{trait_index}", response_model=DataResponse[TraitDetailExtended])
async def get_trait_details(
    trait_index: int,
    include_studies: bool = Query(default=True, description="Include associated studies"),
    include_similar: bool = Query(default=True, description="Include similar traits"),
    include_efo: bool = Query(default=True, description="Include EFO mappings"),
    max_studies: int = Query(default=50, ge=1, le=200, description="Maximum studies to return"),
    max_similar: int = Query(default=10, ge=1, le=50, description="Maximum similar traits"),
    similarity_threshold: float = Query(default=0.3, ge=0.0, le=1.0, description="Similarity threshold"),
    service: TraitService = Depends(get_database_service),
    ...
    try:
        ...
        if not trait:
            raise HTTPException(status_code=404, detail=f"Trait with index {trait_index} not found")
        ...
        logger.error(f"Error getting trait details for index {trait_index}: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to get trait details: {str(e)}")


@router.get("/{trait_index}/studies", response_model=PaginatedDataResponse[List[Dict[str, Any]]])
async def get_trait_studies(
    ...
    date_from: Optional[str] = Query(default=None, description="Publication date from (YYYY-MM-DD)"),
    date_to: Optional[str] = Query(default=None, description="Publication date to (YYYY-MM-DD)"),
    service: TraitService = Depends(get_database_service),
    ...
    try:
        ...
        params: List[Union[int, str]] = [trait_index]
        ...
        WHERE {where_clause}
        """
        total_count = service.study_repo.execute_query(count_query, tuple(params))[0][0]
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get trait studies: {str(e)}")


@router.get("/{trait_index}/similar", response_model=DataResponse[List[SimilaritySearchResult]])
async def get_similar_traits(
    trait_index: int,
    max_results: int = Query(default=10, ge=1, le=100, description="Maximum results to return"),
    similarity_threshold: float = Query(default=0.3, ge=0.0, le=1.0, description="Minimum similarity"),
    service: TraitService = Depends(get_database_service),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to find similar traits: {str(e)}")


@router.get("/{trait_index}/efo-mappings", response_model=DataResponse[List[SimilaritySearchResult]])
async def get_trait_efo_mappings(
    trait_index: int,
    max_results: int = Query(default=10, ge=1, le=50, description="Maximum mappings to return"),
    similarity_threshold: float = Query(default=0.3, ge=0.0, le=1.0, description="Minimum similarity"),
    service: TraitService = Depends(get_database_service),
) -> DataResponse[List[SimilaritySearchResult]]:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get EFO mappings: {str(e)}")
...
async def get_traits_overview(
    ...
    try:
        ...
        ORDER BY 1
        """
        distribution_results = service.trait_repo.execute_query(distribution_query)
        appearance_distribution = {row[0]: row[1] for row in distribution_results}
        ...
        """
        model_coverage_results = service.trait_repo.execute_query(model_coverage_query)
        model_coverage = [
            {
                "model": row[0],
                "unique_traits": row[1],
        ...
        overview = {
            ...
            "average_appearances": round(total_appearances / total_traits, 2) if total_traits > 0 else 0,
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get traits overview: {str(e)}")
...
async def get_traits_bulk(
    ...
    try:
        ...
        traits = [trait for trait in traits if trait is not None]  # Filter out None values
        ...
        raise HTTPException(status_code=500, detail=f"Failed to get bulk traits: {str(e)}")